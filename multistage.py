...
def hash_second_pass (baskets, bitmap, freq):
   buckets = {}
   
   for basket in baskets:
      pairs = list(itertools.combinations(basket, 2))
      for pair in pairs:
         if(pair[0] in freq and pair[1] in freq and bitmap[hash(pair[0], pair[1]) == 1]):
          index = hash1(pair[0], pair[1]) 
          buckets[index] = 1 if index not in buckets else buckets[index]+1
   print('Successful candidate Singleton Itemset creation\n\n')
   return buckets
...
def multistage(dataset_file, threshold):  
  print('\n\nPASS 1 !!!')
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='Multistage Algorithm')
  ...
  print(multistage(args.datafile, int(args.threshold)))
